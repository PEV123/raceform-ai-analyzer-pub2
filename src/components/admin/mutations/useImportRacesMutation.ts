...
// Helper function to format dates consistently in UK timezone
...
export const useImportRacesMutation = () => {
  ...
  return useMutation({
    mutationFn: async (date: Date) => {
      ...
      console.log("Starting import for UK date:", ukFormattedDate);
      ...
      try {
        ...
        for (const race of races) {
          ...
          try {
            ...
            // Parse and format the off_time in UK timezone
            const [hours, minutes] = race.off_time.split(':').map(Number);
            const raceDate = new Date(date);
            raceDate.setHours(hours, minutes, 0, 0);
            
            const ukOffTime = formatUKDate(raceDate);
            console.log('Formatted UK off_time:', ukOffTime);
